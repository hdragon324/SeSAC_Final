        ...
        with st.container():
            ...
            st.markdown('''
                **:blue[✅ 사이드바 메뉴 선택]**  
                ...
                - **AI 사업계획서 초안** : GPT-4o를 활용해 사업계획서 초안을 자동 생성할 수 있습니다.''')
            st.markdown(
...
def register_page():
    '''회원가입 페이지'''
    st.header("👋 회원가입")
    st.write("테스트용 페이지로 전락해버린 회원가입")

    # JSON 파일을 읽어와 딕셔너리로 변환
    with open('data/사업계획서.json', 'r', encoding='utf-8') as file:
        plan = json.load(file)

    result = "\n\n".join(plan.values())
    with st.expander("📜 사업계획서 내용 보기"):
        st.code(result, language="plaintext")

    #-----------------------------------------------------------------------

    # JSON 데이터를 문자열로 변환
    json_data = json.dumps(plan, indent=4, ensure_ascii=False)

    # 다운로드 버튼 생성
    st.download_button(
        label="📥 사업계획서 다운로드",
        data=json_data,
        file_name="사업계획서.json",
    ...
    if st.session_state.selected_template == '예비창업패키지':
        ...
        if st.button("📄 사업계획서 작성하기"):
            with st.spinner("🔄 Loading...잠시만 기다려 주세요!"):
                ...
                pm.execute_notebook(
                    'data/예비창업패키지.ipynb',  # 실행할 Jupyter 노트북 경로
                    'data/output.ipynb',  # 실행 결과를 저장할 출력 파일 경로
    ...
    # template으로 초기창업패키지를 선택한 경우
        ...
        if st.button("📄 사업계획서 작성하기"):
            with st.spinner("🔄 Loading...잠시만 기다려 주세요!"):
                # papermill을 사용하여 노트북 실행
                pm.execute_notebook(
                    'data/초기창업패키지.ipynb',  # 실행할 Jupyter 노트북 경로
                    'data/output.ipynb',  # 실행 결과를 저장할 출력 파일 경로
                    parameters={'loaded_data': item_info}  # item_info 파라미터 전달
                )
    ...
    if st.session_state.selected_template == '청년창업사관학교':
        ...
        if st.button("📄 사업계획서 작성하기"):
            with st.spinner("🔄 Loading...잠시만 기다려 주세요!"):
                # papermill을 사용하여 노트북 실행
                pm.execute_notebook(
                    'data/청년창업사관학교.ipynb',  # 실행할 Jupyter 노트북 경로
                    'data/output.ipynb',  # 실행 결과를 저장할 출력 파일 경로
