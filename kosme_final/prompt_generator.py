def generate_prompt(section, item_info, kosme_template, kosme_pass):
    """
    OpenAI에게 보낼 프롬프트를 생성하는 함수
    """
    section_prompts = {
        "제품 및 서비스 개발동기 및 필요성": f"""
        당신은 창업가의 아이디어를 구체화하고 사업 가능성을 검증하는 사업계획서 작성 전문가입니다. 
        {kosme_template}
        {kosme_pass}
        
        다음 정보를 바탕으로 제품 및 서비스 개발동기와 필요성 부분을 작성해 주세요.

        문제 인식 1: {item_info['제품 및 서비스 개발동기']}
        문제 인식 2: {item_info['제품 및 서비스의 목적_필요성']}
        """,
        
        "개발 방안": f"""
        다음 정보를 바탕으로 제품 및 서비스의 개발 방안 부분을 작성해 주세요.

        실현 가능성 1: {item_info['제품 및 서비스의 개발 방안']}
        """,

        "시장진입 및 자금조달": f"""
        다음 정보를 바탕으로 시장진입 및 자금조달 관련 내용을 작성해 주세요.

        성장 전략 1: {item_info['내수시장 확보 방안 경쟁 및 판매가능성']}
        성장 전략 2: {item_info['해외시장 진출 방안 경쟁 및 판매가능성']}
        성장 전략 3: {item_info['자금 조달 및 개발 방안']}
        """,

        "팀 구성 및 사회적 가치": f"""
        다음 정보를 바탕으로 팀 구성 및 사회적 가치 관련 내용을 작성해 주세요.

        팀 구성 1: {item_info['대표자 및 직원의 보유역량 및 기술보호 노력']}
        팀 구성 2: {item_info['사회적 가치 실천계획']}
        팀 구성 3: {item_info['지역특화 아이디어 기반 사업추진계획']}
        """
    }

    return section_prompts.get(section, "잘못된 요청입니다.")

